...
public class Feed extends FeedBase implements Serializable {
	...
	public void encrypt(String id, Object cipher, String secretkey) {if (entry!=null) for (int i=0;i<entry.size();i++) { ((jp.reflexworks.atom.entry.SoftSchema)entry.get(i)).encrypt(id, cipher, secretkey);}};
	public void decrypt(String id, Object cipher, String secretkey) {if (entry!=null) for (int i=0;i<entry.size();i++) { ((jp.reflexworks.atom.entry.SoftSchema)entry.get(i)).decrypt(id, cipher, secretkey);}};
